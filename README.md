...
- **About** is a two-sentence introduction, three compact ideas (ship real products; design around users and constraints; communicate through presentations and leadership), what I'm looking for, and the tools grouped by where they were used. The 4:5 portrait is Mruthulan's photo from Dell InnovateFest 2026 (event lanyard retouched out at his request); if the file is ever removed from `data/media.json` the slot falls back to a designed monogram, never an empty frame.
...
`data/content.json` holds text only Mruthulan can supply — the personal About paragraph and the Better Call Bhai owner testimonial. Both are now `ready`. The About paragraph is also written straight into `index.html` so it shows without JavaScript; change both together. An entry renders only when its `status` is `ready` and its text is set. Re-run `design/build_cases_v4.py` after changing the testimonial.

## Photos and media

The SignalBridge and MEANT interface frames were captured from each app running locally — SignalBridge on its fictional seed data, MEANT's tablet client on the hawker demo pack — so no real person appears. The Better Call Bhai frames come from the shop's own code run locally against an empty database (the only booking names are Mruthulan's test bookings), set in laptop and phone frames; nothing is booked on the live site and the shop's analytics tag was blocked while capturing. Every filled screenshot opens full size on tap (turned sideways on a portrait phone), because a desktop UI shrunk into a phone column is too small to read.
